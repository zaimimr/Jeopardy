...
- **Numeral** (600 Jost, tabular lining, tracking -0.02em): Points and scores. Board panels `clamp(1.6rem, 4.2vw, 5rem)`; scoreboard plaques `clamp(2rem, 4vw, 4.2rem)` (or `clamp(1.6rem, 3vw, 3.2rem)` with more than five teams); the room code in the QR overlay `clamp(2.4rem, 5vw, 5rem)` bold with 0.2em tracking in brass-light; the phone's room-code plate 1.125rem bold with 0.15em tracking; join field 3rem bold with 0.3em tracking.
...
- **Big-screen header:** Title and italic subtitle left; right, an uppercase status label with a 8px dot (green when live, brass otherwise) and the "Koble til mobil" brass button that opens the QR overlay. Icons are inline SVG outlines at 1.6 to 1.75 stroke in currentColor, never an icon font.
